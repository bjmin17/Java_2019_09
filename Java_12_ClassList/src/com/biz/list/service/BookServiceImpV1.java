...
public class BookServiceImpV1 implements BookService {
	...
	Scanner scan; // 키보드에서 값 입력 받기 위한 준비
	...
	// 아래 input은 도서 1개만 검색하기 때문에 반복하려고 할 때 만드는 method
	...
	public void input() {
		 ...
		 * 
		 * 이 App에서 NumberFormatException은 발생할 확률이
		 * 매우(0.00000001%) 높다.
		 * 
		 ...
		 * 		입력할 경우 안내 메시지를 보여주고, 다음 값을 입력하도록
		 * 		유도하는 코드를 작성
		 * 
		 * 
		 */
		 ...
		 * 예외처리
		 * 사용자가 app을 사용하는 중에
		 * 데이터를 입력하거나 어떤 연산을 수행할
		 * Run time error가 발생할 확률이 1/1000000라도
		 * 있다면 그 app은 사용자에게 상당한 스트레스를 줄 수 있다.
		 * 
		 * 개발자는 그러한 상황을 만들지 않기 위해
		 * 		예측할 수 있는 예외가 발생할 코드에서
		 * 		예외 처리를 해주어야 한다.
		 * 
