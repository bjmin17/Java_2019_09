...
public class IolistServiceImpV1 implements IolistService {
	...
	/*
	 * 접근제한자
	 * public : 누구나 접근 가능한 친구들
	 * private : 나만(현재 클래스내에 있는 method에서만) 접근 가능한 친구들
	 * protected : private 성질을 가지면서 현재 클래스를 상속한 곳에서는 자유롭게 접근할 수 있도록 개방하는 접근
	 * 
	 *  IolistEx_01이나 IolistEx_02에서는 접근 불가!!
	 *  IolistServiceImpV2는 ImpV1을 상속받았으므로 접근 가능
	 *  
	 * 
	 */
	...
	// 자동으로 실행되는 method
	// 1. 멤버변수가 객체 등 작동하는데 필요한 요소들을 초기화하는 코드를 작성
	public IolistServiceImpV1() {
		// 이 코드가 누락되면 input() method에서 NullpointException이 발생
	...
	/*
	 * iolist에 저장된 VO 들의 total값을 계산할 곳
	 */
	@Override
	public void total() {
		...
		for(int i = 0 ; i < nSize ; i++) {
			...
			// vo 변수(객체)에는 iolist의 i번째 vo의 주소가복사된다
			IolistVO vo = iolist.get(i);
			
			// iolist.get(i번째).intPrice를 읽고
			// iolist.get(i번째).intQty를 읽어서 곱셈한 후에
			// total에 잠시 보관
		...
		// 위의 int nSize ~ IolistVO vo 구문까지 세줄의 내용을 for 한줄이 담음
		
		// vo는 iolist 개수만큼 자동 반복 실행되면서
		...
		for(IolistVO vo : iolist) {
			...
			// iolist 현재 위치에 있는 iolistVO의 total변수를
			// 자동으로 찾아가서 값을 세팅해준다.
			vo.setIntTotal(total);
		}
