...
public class BankServiceImpV1 implements BankService {
	...
	public BankServiceImpV1() {
		// TreeMap : key 값을 중심으로 오름차순 자동 정렬
	...
	public void viewAcc() {
		...
		// bankBookList에서 strNum 키 값을 조회
		// 그 결과를 BankBookVO에 담기
//		BankBookVO bookVO = bankBookList.get(strNum); // get method를 이용해서 strNum를 조회
		// bankBookList에서 추출된 bookVO를 입, 출금 처리 method에서
		// 사용할 수 있도록 멤버변수 영역으로 이동하기
		bookVO = bankBookList.get(strNum);
		
		// bookVO에는 계좌 정보나 null 값이 담기게 된다.
		// 계좌정보가 담기는 경우 ? : strNum 계좌가 list에 있을 때
		// null 이 담기는 경우 ? : strNum 계좌가 list에 없을 때
		
		if(bookVO == null) {
			System.out.println("계좌정보가 없음");
		}
		System.out.println("=====================================================");
		System.out.println("계좌번호 : " + bookVO.getNum());
		System.out.println("잔액 : " + bookVO.getBalance());
		System.out.println("최근거래일자 : " + bookVO.getLastDate());
		System.out.println("구분 : " + bookVO.getRemark());
		System.out.println("=====================================================");
	}
	...
	public void input() throws Exception {
		...
		// viewAcc() 에서 유효하지 않은 계좌 정보에 대해서는
		// 이미 메시지를 보여 주었으므로
		// 여기에서는 검사만 하고, 더 이상 진행을 멈추어버리자.
		if(bookVO == null) return; // viewAcc에서 계좌정보가 잘못됐다는걸 알려주기 때문에 바로 종료
		...
		// 여기서 하나의 문제가 발생하는데
		// 저장할 파일 이름을 넘겨줄 방법을 모색해야 한다.
		// input() method를 변경하여
		// input(String bookFile) 형식으로 바꿀 수도 있지만,
		// 그렇게 하면 
		// interface, 그리고 interface를 implements 한 클래스
	...
	public void bookWrite(String bookFile) throws Exception {
		...
		for(String vo : keys) {
			fileOut.printf("%s:%d:%s:%s\n", 
					bankBookList.get(vo).getNum(),
					bankBookList.get(vo).getBalance(),
					bankBookList.get(vo).getLastDate(),
					bankBookList.get(vo).getRemark());
			fileOut.flush();
		}
		fileOut.close();
	}
