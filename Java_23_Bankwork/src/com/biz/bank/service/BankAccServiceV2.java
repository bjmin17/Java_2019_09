...
public class BankAccServiceV2 implements BankAccService {
	 ...
	 * viewAcc() method에서
	 * bankList를
	 * 계좌번호로 조회를 했을 때 해당하는 계좌번호가 조회되면
	...
	public void input() throws Exception {
		...
		if(bankAccVO != null) {
			...
			// DecimalFormat
			// 숫자값 3000 을 = 3,000으로 바꿀 때 사용했던 클래스와
			// 비슷한 기능으로
...
//		for(int i = 0; i < bankList.size(); i++) {
		...
		for(String vo : _s) {
			BankAccVO ba = bankList.get(vo);
			String strNum = ba.getNum();
			int intBal = ba.getBalance();
			String strDate = ba.getLastDate();
			String strRemark = ba.getRemark();
			
			System.out.printf("%s\t%d\t%s\t%s\n", strNum, intBal, strDate, strRemark);
	...
	// 계좌 내용을 파일에 저장
	// 0001:1000:2019-09-23:입금
	public void writeAccBook(String strBookFile) throws Exception {
		PrintWriter fileOut = null;
		fileOut = new PrintWriter(strBookFile);
		
		Set<String> keys = bankList.keySet(); // Map의 key들을 뽑아냄
		for(String vo : keys) {
//			BankAccVO bank = bankList.get(vo);
//			
...
//					bank.getRemark());
			...
			fileOut.printf("%s:%d:%s:%s\n", 
					bankList.get(vo).getNum(), 
					bankList.get(vo).getBalance(), 
					bankList.get(vo).getLastDate(), 
					bankList.get(vo).getRemark());
